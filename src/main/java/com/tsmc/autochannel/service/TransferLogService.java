...
public class TransferLogService {
    ...
    public TransferLog logStart(String channelId, OperationType operation, String filename) {
        ...
        try {
            ...
            obs.lowCardinalityKeyValue(MariaDbObservationTag.error.name(), classifyJpaError(e));
            obs.error(e);
            throw e;
        } finally {
            obs.stop();
    ...
    public List<TransferLog> queryByStatus(LogStatus status) {
        Observation obs = Observation.createNotStarted("mariadb", observationRegistry)
                ...
                .start();
        try {
            List<TransferLog> result = repository.findByStatus(status);
    ...
    public long countByChannelAndStatus(String channelId, LogStatus status) {
        Observation obs = Observation.createNotStarted("mariadb", observationRegistry)
                ...
                .start();
        try {
            long count = repository.countByChannelIdAndStatus(channelId, status);
    ...
    public long sumFileSizeByChannel(String channelId) {
        Observation obs = Observation.createNotStarted("mariadb", observationRegistry)
                ...
                .start();
        try {
            long total = repository.sumFileSizeByChannelId(channelId);
    ...
    private String classifyJpaError(DataAccessException e) {
